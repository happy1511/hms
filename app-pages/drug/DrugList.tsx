...
const Actions = ({
  ...
  const { mutateAsync: deleteDrug, isPending: deletePending } = useDeleteDrug();
    ...
    <>
      ...
      {canDelete && (
        <CustomAlert
          ...
          title="Delete Drug?"
          description="Are you sure you want to delete drug?"
          cancelText="Cancel"
          confirmText="Delete"
          handleConfirm={() => deleteDrug({ drugId: Number(data.id) })}
...
const DrugList = () => {
  ...
  const columns: ColumnDefWithClass<PharmacyDrugType>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Link
          href={canUpdate ? `/drug/${row.original.id}` : "#"}
