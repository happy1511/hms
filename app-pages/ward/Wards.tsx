...
import { useDeleteWard, useWardsList } from "@/hooks/query/ward";
import {
  ColumnDefWithClass,
  FilterConfig,
  FilterValues,
...
const Buttons = ({ canCreate }: { canCreate: boolean }) => {
  ...
  return (
    <>
      {canCreate && (
        <CustomButton onClick={() => router.push("/wards/new")}>
          New Ward
        </CustomButton>
      )}
    </>
...
const Actions = ({
  ...
  const { mutateAsync: deleteWard, isPending: deletePending } = useDeleteWard();

  return (
    <>
      {canView && (
        <DataViewModal<WardType>
          data={data}
          title="Ward Details"
          fields={[
            { key: "id", label: "WardId" },
      ...
      {canDelete && (
        <CustomAlert
          ...
          cancelText="Cancel"
          confirmText="Delete"
          handleConfirm={() => deleteWard({ wardId: Number(data.id) })}
...
const Wards = () => {
  ...
  const { data, isLoading, isError, error } = useWardsList(
    filters,
    page,
    limit,
  );
  ...
  const columns: ColumnDefWithClass<WardType>[] = [
    {
      accessorKey: "id",
      header: ({ column }) => {
        return <SortableHeader<WardType> label="ID" column={column} />;
      },
    ...
    },
    {
      accessorKey: "name",
      header: ({ column }) => {
        return <SortableHeader<WardType> label="Ward Name" column={column} />;
      },
      cell: ({ row }) => (
        <Link
          href={canUpdate ? `/wards/${row.original.id}` : "#"}
    ...
    },
    {
      accessorKey: "floor",
      header: ({ column }) => {
        return <SortableHeader<WardType> label="Floor Name" column={column} />;
      },
    ...
    },
    {
      ...
      header: ({ column }) => {
        return <SortableHeader<WardType> label="Created at" column={column} />;
      },
      cell: ({ row }) => {
        return (
          <div className="flex">
    ...
    },
    {
      ...
      header: ({ column }) => {
        return <SortableHeader<WardType> label="Updated at" column={column} />;
      },
      cell: ({ row }) => {
        return (
          <div className="flex">
