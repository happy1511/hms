...
import { useDeleteRoomType, useRoomTypeList } from "@/hooks/query/roomType";
    ...
    <>
      {canCreate && (
        <div className="flex items-center gap-4">
          <CustomButton onClick={() => router.push("/room-types/new")}>
            New Room Type
          </CustomButton>
          <MasterImportModal master="room-type" allowReplace={canDelete} />
        </div>
      )}
    </>
  );
...
const Actions = ({
  ...
  const { mutateAsync: deleteWard, isPending: deletePending } =
    useDeleteRoomType();

  return (
    <>
      {canView && (
        <DataViewModal<RoomTypeGetPayload<{ include: { department: true } }>>
          data={data}
  ...
  >[] = [
    {
      ...
      header: ({ column }) => {
        return (
          <SortableHeader<RoomTypeGetPayload<{ include: { department: true } }>>
            label="ID"
            column={column}
          />
        );
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Link
          href={canUpdate ? `/room-types/${row.original.id}` : "#"}
