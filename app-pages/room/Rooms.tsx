...
import { useDeleteRoom, useRoomsList } from "@/hooks/query/room";
    ...
    <>
      {canCreate && (
        <div className="flex items-center gap-4">
          <CustomButton onClick={() => router.push("/room/new")}>
            New Room
          </CustomButton>
          <MasterImportModal master="room" allowReplace={canDelete} />
        </div>
      )}
    </>
  );
...
const Actions = ({
  ...
  const { mutateAsync: deleteWard, isPending: deletePending } = useDeleteRoom();

  return (
    <>
      {canView && (
        <DataViewModal<RoomGetPayload<{ include: { roomType: true } }>>
          data={data}
          title="Room Details"
          fields={[
            { key: "id", label: "RoomId" },
      ...
      {canDelete && (
        <CustomAlert
          ...
          cancelText="Cancel"
          confirmText="Delete"
          handleConfirm={() => deleteWard({ roomId: Number(data.id) })}
...
const Rooms = () => {
  ...
  const { data, isLoading, isFetching, refetch, isError, error } = useRoomsList(
    filters,
    page,
    limit,
  );
  ...
  >[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Link
          href={canUpdate ? `/room/${row.original.id}` : "#"}
