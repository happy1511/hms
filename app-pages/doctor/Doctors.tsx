    ...
    <>
      {canCreate && (
        <div className="flex items-center gap-4">
          ...
          <MasterImportModal master="doctor" allowReplace={canDelete} />
        </div>
      )}
    </>
  );
...
const Doctors = () => {
  ...
  const columns: ColumnDefWithClass<Doctor>[] = [
    ...
    },
    {
      ...
      header: () => {
        return <button className="flex">Doctor Type</button>;
      },
      cell: ({ row }) => (
        <span className="capitalize">{row.original.doctorType}</span>
      ),
