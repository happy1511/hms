...
import WardsBadge from "@/components/floor/WardsBadge";
import { Button } from "@/components/ui/button";
import { ActionType, ModuleType, Status } from "@/generated/prisma/enums";
import { useProfile } from "@/hooks/query/auth";
import { useDeleteFloor, useFloorsList } from "@/hooks/query/floor";
import {
  ColumnDefWithClass,
  FilterConfig,
  FilterValues,
...
const Buttons = ({ canCreate }: { canCreate: boolean }) => {
  ...
  return (
    <>
      {canCreate && (
        <CustomButton onClick={() => router.push("/floors/new")}>
          New Floor
        </CustomButton>
      )}
    </>
...
const Actions = ({
  ...
  return (
    <>
      {canView && (
        <DataViewModal<FloorType>
          data={data}
          title="Floor Details"
          fields={[
            { key: "id", label: "FloorId" },
            { key: "name", label: "Name" },
      ...
      {canDelete && (
        <CustomAlert
          ...
          cancelText="Cancel"
          confirmText="Delete"
          handleConfirm={() => deleteFloor({ floorId: Number(data.id) })}
...
const Floors = () => {
  ...
  const { data, isLoading, isError, error } = useFloorsList(
    filters,
    page,
    limit,
  );
  ...
  const columns: ColumnDefWithClass<FloorType>[] = [
    {
      accessorKey: "id",
      header: ({ column }) => {
        return <SortableHeader<FloorType> label="ID" column={column} />;
      },
    ...
    },
    {
      accessorKey: "name",
      header: ({ column }) => {
        return <SortableHeader<FloorType> label="Floor Name" column={column} />;
      },
      cell: ({ row }) => (
        <Link
          href={canUpdate ? `/floors/${row.original.id}` : "#"}
    ...
    },
    {
      ...
      header: ({ column }) => {
        return <SortableHeader<FloorType> label="Wards Name" column={column} />;
      },
      cell: ({ row }) => <WardsBadge wards={row.original.departments || []} />,
      headerClassName: "min-w-50",
      cellClassName: "min-w-50",
    },
    ...
    },
    {
      ...
      header: ({ column }) => {
        return <SortableHeader<FloorType> label="Created at" column={column} />;
      },
      cell: ({ row }) => {
        return (
          <div className="flex">
    ...
    },
    {
      ...
      header: ({ column }) => {
        return <SortableHeader<FloorType> label="Updated at" column={column} />;
      },
      cell: ({ row }) => {
        return (
          <div className="flex">
