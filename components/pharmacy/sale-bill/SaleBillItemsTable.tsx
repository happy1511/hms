...
const SaleBillItemRow = ({
  ...
  useEffect(() => {
    ...
    const taxableAmount = Number((quantity * selectedRate).toFixed(2));
    const cGstAmount = Number(
      ((taxableAmount * Number(inventory.drug.cGstPercentage || 0)) / 100).toFixed(
        2,
      ),
    );
    const sGstAmount = Number(
      ((taxableAmount * Number(inventory.drug.sGstPercentage || 0)) / 100).toFixed(
        2,
      ),
    );
    const iGstAmount = Number(
      ((taxableAmount * Number(inventory.drug.iGstPercentage || 0)) / 100).toFixed(
        2,
      ),
    );
    const fallbackGst = Number(
      ((taxableAmount * Number(inventory.drug.gstPercentage || 0)) / 100).toFixed(
        2,
      ),
    );
    const gstAmount =
    ...
    setIfChanged(ratePath, selectedRate);
  ...
  return (
    <tr className="border-t align-top">
      ...
      <td className="px-2 py-1 min-w-72">
        <FormInfiniteSelect<
      ...
      <td className="px-2 py-1">{inventory?.drug.manufacturer || "-"}</td>
      <td className="px-2 py-1">{inventory?.drug.hsnCode || "-"}</td>
      <td className="px-2 py-1">{inventory?.batchNo || "-"}</td>
      <td className="px-2 py-1">
        {inventory ? format(new Date(inventory.expiryDate), "MM/yy") : "-"}
      </td>
      <td className="px-2 py-1">{inventory?.drug.unit || "-"}</td>
      ...
      <td className="px-2 py-1">{Number(item?.rate || 0).toFixed(2)}</td>
      <td className="px-2 py-1">{inventory?.drug.cGstPercentage || 0}%</td>
      <td className="px-2 py-1">{inventory?.drug.sGstPercentage || 0}%</td>
      <td className="px-2 py-1">{inventory?.drug.iGstPercentage || 0}%</td>
      <td className="px-2 py-1">{Number(item?.discountValue || 0).toFixed(2)}</td>
